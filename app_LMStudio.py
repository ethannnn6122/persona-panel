...
LM_STUDIO_API_URL = "http://localhost:1234/v1/responses"
...
def get_model_response(model_name, prompt):
    ...
    with st.spinner(f"Querying {model_name} for its thoughts..."):
        payload = {"model": model_name, "input": prompt, "stream": False}
        try:
            ...
            if model_output and isinstance(model_output, list) and len(model_output) > 0:
                content_list = model_output[0].get("content")
                if content_list and isinstance(content_list, list) and len(content_list) > 0:
                    response_text = content_list[0].get("text")
                    if response_text:
                        return response_text
            st.error("Error: Could not find model text in the expected nested JSON structure.")
            return None
        except requests.exceptions.Timeout:
            st.error(f"Error: The request to {model_name} timed out.")
            return None
        except requests.exceptions.RequestException as e:
            st.error(f"Error communicating with API for {model_name}: {e}")
            return None

@st.cache_data(show_spinner=True)
def get_available_models(refresh_trigger):
    """
    Try to query the local API for installed models.
    """
    ...
    try:
        ...
        resp = requests.get(f"{LMSTUDIO_BASE_URL}/v1/models")
        try:
            if not resp.ok:
